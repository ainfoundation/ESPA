...
import { Search, Save, Edit, X, Calendar, Clock, MapPin, Download, MoreVertical, Plus, PlaneLanding, PlaneTakeoff, Trash2 } from 'lucide-react';
          ...
          <DraggableModal className="bg-white rounded-3xl p-8 w-full max-w-2xl max-h-[90vh] shadow-2xl flex flex-col animate-in zoom-in-95 duration-200 pointer-events-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-8 flex-1 overflow-y-auto min-h-0 pr-2 -mr-2">
              ...
              <div className="space-y-4">
                ...
                {viewingUser.arrivalConnectingFlights?.length > 0 && (
                  <div className="mt-4 space-y-3 pl-4 border-l-2 border-stone-100">
                    <h5 className="text-xs font-bold text-stone-600 uppercase">CONNECTION</h5>
                    {viewingUser.arrivalConnectingFlights.map((flight, idx) => (
                      <div key={idx} className="bg-stone-50/50 p-5 rounded-xl border border-stone-100 space-y-4">
                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 justify-between relative">
                          <div className="flex-1">
                            <span className="block text-[10px] font-medium text-stone-500 uppercase tracking-wider mb-1">From</span>
                            <div className="text-sm font-medium text-stone-900 flex flex-col gap-1">
              ...
              <div className="space-y-4">
                ...
                {viewingUser.departureConnectingFlights?.length > 0 && (
                  <div className="mt-4 space-y-3 pl-4 border-l-2 border-stone-100">
                    <h5 className="text-xs font-bold text-stone-600 uppercase">CONNECTION</h5>
                    {viewingUser.departureConnectingFlights.map((flight, idx) => (
                      <div key={idx} className="bg-stone-50/50 p-5 rounded-xl border border-stone-100 space-y-4">
                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 justify-between relative">
                          <div className="flex-1">
                            <span className="block text-[10px] font-medium text-stone-500 uppercase tracking-wider mb-1">From</span>

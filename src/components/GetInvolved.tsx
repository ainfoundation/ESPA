    ...
    <section id="get-involved" className="bg-[#004B36] py-16 md:py-24 transition-colors duration-300">
      <div className="max-w-6xl mx-auto px-4 md:px-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {cards.map((card, i) => (
            <div key={i} className="bg-[#003828] p-6 md:p-12 lg:p-8 rounded-3xl shadow-sm border border-white/5 flex flex-col h-full overflow-hidden transition-colors duration-300">
              ...
              <p className="text-justify font-sans text-white/70 mb-8 flex-grow text-lg lg:text-base font-normal transition-colors duration-300">{card.desc}</p>
              <button onClick={(e) => { e.preventDefault(); window.dispatchEvent(new CustomEvent(card.action)); }} className="w-full bg-white text-[#004B36] py-4 hover:bg-neutral-200 rounded-full font-bold hover:bg-[#003828] :bg-neutral-200 transition-colors">
                {card.btn}
              </button>
            </div>
          ))}

...
// 1. Update MemberListView.jsx for search bar height
let memberList = fs.readFileSync('src/components/MemberListView.jsx', 'utf8');
memberList = memberList.replace(
  'className="w-full pl-10 pr-4 py-2 border border-stone-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#004B36] text-sm"',
...
fs.writeFileSync('src/components/MemberListView.jsx', memberList);

// 2. Rewrite SummaryDashboard.jsx
      ...
      <div className="space-y-8 h-full flex flex-col tracking-tight relative p-4 md:p-8 overflow-y-auto">
        <div>
          <h1 className="text-3xl font-semibold text-stone-900 flex items-center gap-2">
            <LayoutDashboard className="text-[#004B36]" size={28} /> Dashboard
          </h1>
    ...
    <div className="space-y-8 h-full flex flex-col tracking-tight relative p-4 md:p-8 overflow-y-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl border border-stone-200/60 shadow-sm p-6 flex flex-col items-center justify-center text-center">
          ...
          <div className="text-4xl font-bold text-stone-900">Rs {funds.pkr.toLocaleString()}</div>
        </div>
        <div className="bg-white rounded-2xl border border-stone-200/60 shadow-sm p-6 flex flex-col items-center justify-center text-center">
          <Wallet size={32} className="text-[#004B36] mb-4" />
        ...
        <div className="bg-stone-50 rounded-2xl border border-stone-200/60 shadow-sm p-6">
          ...
          <div className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="text-stone-900 font-bold">Rs {totalDonationsPKR.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
        ...
        <div className="bg-stone-50 rounded-2xl border border-stone-200/60 shadow-sm p-6">
          ...
          <div className="space-y-2">
            <div className="flex justify-between">
              <span className="text-stone-600 font-medium">PKR</span>
              <span className="text-stone-900 font-bold">Rs {totalAllocationsPKR.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-stone-600 font-medium">USD</span>
...
fs.writeFileSync('src/components/SummaryDashboard.jsx', dashboardCode.replace(/\\\\\\$\\\\\\{/g, '${').replace(/\\}\\}/g, '}'));

// 3. Create FundsView.jsx
const fundsCode = `import React, { useState } from 'react';
  ...
  return (
    <div className="space-y-8 h-full flex flex-col tracking-tight relative p-4 md:p-8 overflow-hidden">
      <div className="shrink-0 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h1 className="text-3xl font-semibold text-stone-900 flex items-center gap-2">
            <Banknote className="text-[#004B36]" size={28} /> Funds & Allocations
          </h1>
...
fs.writeFileSync('src/components/FundsView.jsx', fundsCode.replace(/\\\\\\$\\\\\\{/g, '${').replace(/\\\\\\$\\ /g, '$ ').replace(/\\}\\}/g, '}'));
